...
# When running in Azure Container Apps with a User-Assigned Managed Identity,
# AZURE_CLIENT_ID must be set to the identity's client ID so that
# DefaultAzureCredential can locate the correct managed identity.
# Locally, AZURE_CLIENT_ID is typically unset and the credential falls back
# to AzureCLI / VS Code / environment-based auth.
...
if __name__ == "__main__":
    ...
    with tracer.start_as_current_span(connection_message):
        ...
        uvicorn.run("chat_app_3_multiAgent:app", host="0.0.0.0", port=port)
